...
;(function($) {
    ...
    function handleSliderChange($input, store, context) {
        ...
        if ($slider.find(".is-dragged").length == 0) {
            store.setItem("Agreeableness", newValue);
    ...
    var AgreeablenessRenderer = new Class({
            ...
            popoverTemplate:
                '<ul class="coral-SelectList is-visible is-inline">' +
                    '<div class="coral-Slider coral-Slider--filled" data-init="slider">' +
                        '<label><input id="contexthub-cart-total-slider" min="0" max="100" value="{{agreeableness}}" type="range"/></label>' +
                    '</div>' +
                    '<div class="coral-Tooltip coral-Tooltip--info coral-Tooltip--positionRight" data-init="tooltip" data-interactive="false" data-target="#contexthub-cart-total-slider">{{agreeableness}}</div>' +
                '</ul>'
        },
        ...
        getTemplateContext: function(config) {
            ...
            context.agreeableness = store.getItem("Agreeableness") || 0;
    ...
    ContextHub.UI.registerRenderer("watson.agreeableness", new AgreeablenessRenderer());
